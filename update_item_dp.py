...
key = {"Domain": "telekom.de", "Subdomain": "myfew"}
...
for host in scanned_hosts:
    for hostname in host.hostnames:
        ...
        for k, v in open_in_db.items():
            ...
            del(scanned_ports[0])  # TODO remove
        ...
        for port in open_ports:
            open_in_db_new.update({str(port[0]): {"nmap_service_desc": str(nmap_report.hosts[0].get_service(port[0], protocol=port[1])), "date": host.endtime, "proto": port[1]}})
        ...
        for k, v in open_in_db.items():
            try:
                ...
                open_in_db_new.update({k, v})
        #item.update({"ports_open": open_in_db_new})
        # item.update({"ports-open": {}})  # TODO wrong
        item.update({"scanned_ports": scanned_ports_db})  # TODO append scanned_ports_db
